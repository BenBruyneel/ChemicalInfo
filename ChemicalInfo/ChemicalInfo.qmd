...
generalInfoTable <- data.frame(parameter =  c("Name","CAS","PubChem ID","Formula","Exact Mass","Average Mass","Smiles"),
                               value = c(ifelse(is.null(info$name),
                                         ...
                                         ifelse(identical(info$MolecularFormula, NA),
                                                "", stringToFormula(info$MolecularFormula) %>% formulaToMass(enviPat = TRUE) %>% BBPersonalR::formatDigits(4)()),
                                         ifelse(identical(info$MolecularFormula, NA),
                                                "", stringToFormula(info$MolecularFormula) %>% formulaToMass(enviPat = TRUE, exact = FALSE) %>% BBPersonalR::formatDigits(4)()),
